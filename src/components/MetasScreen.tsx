...
export const MetasScreen: React.FC<MetasScreenProps> = ({
  ...
  // Form de cadastro de venda (Modal e Ações)
  // Campos solicitados: Pedido sem por #, Nome do Cliente/Razão Social (com busca automática), Whatsapp, Valor, Tipo de Cliente (com seta selecionável)
  const [formPedido, setFormPedido] = useState<string>('1052');
  ...
  const handleSalvarVenda = (e: React.FormEvent) => {
    ...
    const newSale: VendaItem = {
      ...
      data: `${ano}-${mes}-${dia}`,
      responsavel: currentUserName || 'Vanessa Gomes',
      vendedor: currentUserName || 'Vanessa Gomes',
      criadoPor: currentUserName || 'Vanessa Gomes',
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto space-y-4 animate-in zoom-in-95 duration-150 text-slate-800">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Lançamento de venda para a meta mensal</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSalvarVenda} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="R$ 0,00"
                      className="w-full pl-10 pr-4 py-2.5 rounded-2xl border border-slate-200 bg-slate-50/70 text-slate-900 text-sm font-bold focus:outline-none focus:border-[#0052cc] focus:bg-white transition-all shadow-2xs"
                    />
                  </div>
                </div>
              </div>

              {/* 5. Tipo de Cliente (seta selecionável) */}
              <div>
                <label className="block text-xs font-bold text-[#0B2046] mb-1 uppercase tracking-wider">
                  Tipo de Cliente
                </label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setFormDesconto(digits ? (parseFloat(digits) / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : '');
                    }}
                    placeholder="R$ 0,00"
                    className="w-full px-4 py-2.5 rounded-2xl border border-slate-200 bg-slate-50/70 text-slate-900 text-sm font-semibold focus:outline-none focus:border-[#0052cc] focus:bg-white transition-all shadow-2xs"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const digits = e.target.value.replace(/\D/g, '');
                      setFormFrete(digits ? (parseFloat(digits) / 100).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : '');
                    }}
                    placeholder="R$ 0,00"
                    className="w-full px-4 py-2.5 rounded-2xl border border-slate-200 bg-slate-50/70 text-slate-900 text-sm font-semibold focus:outline-none focus:border-[#0052cc] focus:bg-white transition-all shadow-2xs"
                  />
              ...
              {/* 7. Forma de Pagamento e Parcelas */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[#0B2046] mb-1 uppercase tracking-wider">
                    Forma de Pagamento
              ...
              {/* 8. Produtos e Valores Unitários */}
