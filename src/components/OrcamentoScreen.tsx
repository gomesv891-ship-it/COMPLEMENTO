...
export const OrcamentoScreen: React.FC<OrcamentoScreenProps> = ({
  ...
  const [descontoInput, setDescontoInput] = useState<string>(
    initialOrcamento?.descontoValor !== undefined
      ? formatPriceToBrl(initialOrcamento.descontoValor)
      : '0,00'
  );
  ...
  const isBudgetInProgress = useMemo(() => {
    ...
    const hasDesconto = Boolean(descontoInput && descontoInput !== '0,00' && descontoInput !== '');
  ...
  const descontoValor = useMemo(() => {
    ...
    return rawVal;
  }, [subtotal, descontoInput, descontoTipo]);
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-10 py-6 sm:py-8 space-y-8">
      ...
      <section className="no-print bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 sm:p-7 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Configuração de Desconto (Interno) */}
          <div className="space-y-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-800 flex items-center gap-2">
                <Percent className="w-4 h-4 text-[#0057ff]" />
                <span>Desconto Comercial (Cálculo Interno)</span>
              </span>
            ...
            <div className="space-y-1 pt-1">
              ...
              <input
                ...
                placeholder="0,00"
                className="w-full h-10 rounded-lg border border-slate-200 bg-white px-3 text-xs sm:text-sm font-bold text-slate-900 outline-none focus:border-blue-500"
              />
            </div>

            <div className="text-[11px] text-slate-500 pt-1 leading-normal">
              <strong>Regra Oficial:</strong> O desconto é calculado
              internamente (Subtotal - Desconto + Frete) e{' '}
              <strong>NÃO</strong> é mostrado no documento impresso/espelho,
              exibindo apenas o valor final consolidado.
            </div>
          </div>
        </div>
