...
/**
 * Realiza os cálculos do simulador Tarkett
 */
export function calcularSimuladorTarkett(params: {
  ...
  custoLiquido: number;
  clienteFinal: { precoVenda: number; lucro: number; margemEfetiva: number };
  revenda: { precoVenda: number; lucro: number; margemEfetiva: number };
  construtora: { precoVenda: number; lucro: number; margemEfetiva: number };
} {
  ...
  const calcPerfil = (margemPercent: number) => {
    ...
    return { precoVenda, lucro, margemEfetiva };
  };
